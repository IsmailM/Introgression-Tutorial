...
* **Download the Ruby script convert\_to\_newick.rb** from [https://rawgit.com/mmatschiner/Introgression-Tutorial/master/scripts/convert_to_newick.rb](https://rawgit.com/mmatschiner/Introgression-Tutorial/master/scripts/convert_to_newick.rb) and place it in the directory that you use for this tutorial. This script takes a single command-line argument as input, which should be the name of a summary-tree file written by TreeAnnotator. As its name implies, the script convert\_to\_newick.rb will convert this file to Newick format (note that the old file will be overwritten).
...
* In order to prepare the input files for PhyloNet, please **share the summary-tree files** for the alignment blocks that you chose to analyse with other participants, so that everyone has a tree set that includes as many local phylogenies as possible.
